...
def add(power2,coeff2,power1,coeff1)
result= Array.new
solution=""
c=0
i=0
...
@len=power1.length
   while c<=@len-1
result << coeff1[c].to_i+ coeff2[c].to_i
           c+=1
        end
  while l<=@len-1
      if result[l]>0
   ...
   while i<=@len-1
solution +=result[i].to_s+"x^"+ power1[i].to_s
           i+=1
        end
 puts solution
end
def subtract(power2,coeff2,power1,coeff1)
result= Array.new
solution=""
c=0
i=0
...
poly2=obj2.create()
obj2.add(power2,coeff2,power1,coeff1)
  elsif option==3
...
(power2,coeff2)=obj2.arrange()
poly2=obj2.create()
  obj2.subtract(power2,coeff2,power1,coeff1)
elsif option==4
puts "Enter your 2nd Polynomial Function with same degree and same number of terms "
function2=gets
obj2.split(function2)
